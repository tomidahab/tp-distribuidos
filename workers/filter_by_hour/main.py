...
# ACK queues for receiving acknowledgments
...
def send_without_ack(exchange, message, routing_key, expected_rows):
    """Send message to filter_by_amount (ACK disabled)"""
    # Send the message
    exchange.send(message, routing_key=routing_key)
    print(f"[filter_by_hour] Worker {WORKER_INDEX} sent message with {expected_rows} rows to {routing_key} (no ACK)", flush=True)

def create_ack_queue(queue_name):
    """Create or get ACK queue for receiving acknowledgments"""
    if queue_name not in ack_queues:
        print(f"[filter_by_hour] Worker {WORKER_INDEX} creating ACK queue: {queue_name}", flush=True)
        try:
            ack_queue = MessageMiddlewareQueue(RABBITMQ_HOST, queue_name)
            ack_queues[queue_name] = ack_queue
            print(f"[filter_by_hour] Worker {WORKER_INDEX} successfully created ACK queue: {queue_name}", flush=True)
            return ack_queue
        except Exception as e:
            print(f"[filter_by_hour] Worker {WORKER_INDEX} ERROR creating ACK queue {queue_name}: {e}", flush=True)
            return None
    else:
        print(f"[filter_by_hour] Worker {WORKER_INDEX} using existing ACK queue: {queue_name}", flush=True)
...
def cleanup_ack_queues():
    """Clean up all ACK queues"""
    for queue_name, queue in ack_queues.items():
        print(f"[filter_by_hour] Worker {WORKER_INDEX} cleaning up ACK queue: {queue_name}", flush=True)
        _close_queue(queue)
    ack_queues.clear()
...
def on_message_callback(message: bytes, filter_by_amount_exchange, categorizer_q3_topic_exchange, categorizer_q3_fanout_exchange, should_stop):
    ...
    if (len(filtered_rows) != 0) or (is_last == 1):
        ...
        if type_of_message == CSV_TYPES_REVERSE['transactions']:  # transactions
            ...
            if filtered_rows:
                ...
                # Send batched messages to each worker with ACK
                for routing_key, worker_rows in rows_by_worker.items():
                    if worker_rows:
                        sender_id = f"filter_by_hour_worker_{WORKER_INDEX}"
                        # Generate unique message ID for ACK tracking
                        message_id = f"{sender_id}_{client_id}_{int(time.time() * 1000000)}"  # microseconds for uniqueness
                        new_message, _ = build_message(client_id, type_of_message, 0, worker_rows, sender_id, message_id)
                        
                        ack_received = send_with_ack_wait(filter_by_amount_exchange, new_message, routing_key, len(worker_rows), message_id)
            ...
            if filtered_rows:  # Only process if there are rows
                ...
                for semester_key, semester_rows in rows_by_semester.items():
                    if semester_rows:
                        semester_message, _ = build_message(client_id, type_of_message, 0, semester_rows, None, None)
    ...
    if is_last == 1 and client_end_messages[client_id] >= NUMBER_OF_YEAR_WORKERS:
        if client_id not in completed_clients:
            ...
            try:
                # Send END to filter_by_amount (to all workers) for this specific client (with ACK)
                sender_id = f"filter_by_hour_worker_{WORKER_INDEX}"
                for i in range(NUMBER_OF_AMOUNT_WORKERS):
                    ...
                    ack_received = send_with_ack_wait(filter_by_amount_exchange, end_message, routing_key, 0, end_message_id)
                    if ack_received:
                        print(f"[filter_by_hour] Worker {WORKER_INDEX} successfully sent END message for client {client_id} to filter_by_amount worker {i} (ACK received)", flush=True)
                    else:
                        print(f"[filter_by_hour] Worker {WORKER_INDEX} WARNING: END message for client {client_id} to filter_by_amount worker {i} failed (no ACK)", flush=True)
...
def main():
    ...
    try:
        ...
        # ACK functionality temporarily disabled
