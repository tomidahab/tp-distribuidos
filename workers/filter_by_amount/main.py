...
# ACK queues for sending acknowledgments  
...
def cleanup_ack_queues():
    ...
    for queue_name, queue in ack_queues.items():
        print(f"[filter_by_amount] Worker {WORKER_INDEX} cleaning up ACK queue: {queue_name}", flush=True)
        _close_queue(queue)
    ack_queues.clear()
...
def create_ack_queue(sender_id):
    """Create or get ACK queue for sending acknowledgments to specific sender"""
    ack_queue_name = f"ack_{sender_id}"
    if ack_queue_name not in ack_queues:
        print(f"[filter_by_amount] Worker {WORKER_INDEX} creating ACK queue: {ack_queue_name} for sender: {sender_id}", flush=True)
        try:
            ack_queue = MessageMiddlewareQueue(RABBITMQ_HOST, ack_queue_name)
            ack_queues[ack_queue_name] = ack_queue
            print(f"[filter_by_amount] Worker {WORKER_INDEX} successfully created ACK queue: {ack_queue_name}", flush=True)
            return ack_queue
        except Exception as e:
            print(f"[filter_by_amount] Worker {WORKER_INDEX} ERROR creating ACK queue {ack_queue_name}: {e}", flush=True)
            return None
    else:
        print(f"[filter_by_amount] Worker {WORKER_INDEX} using existing ACK queue: {ack_queue_name}", flush=True)
...
def on_message_callback(message: bytes, topic_middleware, should_stop):
    ...
    message_id = parsed_message.get('message_id')  # Optional field for ACK
    
    # Send ACK if needed (simplified)
