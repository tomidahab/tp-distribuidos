...
# test_filter_by_year_kill.sh
# Script para probar la tolerancia a fallos matando un worker de filter_by_year
# y luego levantándolo de nuevo

# Configuración
WORKER_NAME="${1:-filter_by_year_worker_1}"  # Worker a matar (por defecto el 0)
...
if ! docker ps --format '{{.Names}}' | grep -q "^${WORKER_NAME}$"; then
    ...
    docker ps --format "table {{.Names}}	{{.Status}}" | grep filter_by_year || echo "Ningún worker filter_by_year corriendo"
    exit 1
fi
...
docker ps --format "table {{.Names}}	{{.Status}}" | grep filter_by_year
...
if [[ $WORKER_NAME =~ filter_by_year_worker_([0-9]+) ]]; then
    WORKER_INDEX="${BASH_REMATCH[1]}"
    SERVICE_NAME="filter_by_year_worker_${WORKER_INDEX}"
